'''
############################ LOFTI Position Vector w/ Gaia ##############################
                              written by Logan Pearce, 2019
#########################################################################################
    Perform Logan's version of OFTI using one observation date and observations of 
    relative position, velocity, or acceleration.  This version retrieves the relevant data
    by querying the Gaia database.

# Requires:
#   python packages astropy, numpy, mpi4py, astroquery
#
# Input:
...
example:
    ...
    ibrun python lofti_phasespace.py <- sbatch command to run LOFTI on TACC.  For Lonestar 5, set nodes=1
            and cores = 48
    mpiexec - 12 python lofti_phasespace_astroquery.py 2100451630105041152 2100451630105040256 -ma 1.2 0.03 -mb 0.8 0.01
       ^ run LOFTI by retrieving the data directly from Gaia DR2 for these two Gaia DR2 source id's with mass of A and mass of B
           specified
'''
...
# ^ (Astropy tables returned by astroquery generate
# a load of warnings that have no bearing on the
# function of this script, so let's just supress
# them...)
...
parser.add_argument("sourceA", help="Gaia DR 2 source id for object A; fit will be of B relative to A",type=str)
parser.add_argument("sourceB", help="Gaia DR 2 source id for object B",type=str)
parser.add_argument("-ma","--mass_a", help="Tuple of mass and error on A separated by a space ex: 1.02 0.2 ", nargs = 2, type = float)
parser.add_argument("-mb","--mass_b", help="Tuple of mass and error on B separated by a space ex: 1.02 0.2 ", nargs = 2, type = float)
args = parser.parse_args()

sourceA = args.sourceA
sourceB = args.sourceB

job = Gaia.launch_job("SELECT * FROM gaiadr2.gaia_source WHERE source_id = "+sourceA+" \
    OR source_id = "+sourceB)

j = job.get_results()

plxa, plxaerr = j[0]['parallax'], j[0]['parallax_error']
plxb, plxberr = j[1]['parallax'], j[1]['parallax_error']

if args.mass_a:
    mA, mAerr = np.float(args.mass_a[0]),np.float(args.mass_a[1])
else:
    mass = tuple(map(float,raw_input('Enter mass of object A and error separated by a space (ex: 1.02 0.2):').split(' ')))
    mA, mAerr = np.float(mass[0]),np.float(mass[1])
if args.mass_b:
    mB, mBerr = np.float(args.mass_b[0]),np.float(args.mass_b[1])
else:
    mass = tuple(map(float,raw_input('Enter mass of object B and error separated by a space (ex: 1.02 0.2):').split(' ')))
...
RAb, RAberr = j[1]['ra'], j[1]['ra_error']*mas_to_deg
Deca, Decaerr = j[0]['dec'], j[0]['dec_error']*mas_to_deg
Decb, Decberr = j[1]['dec'], j[1]['dec_error']*mas_to_deg

# Proper motions (in mas/yr):

pmRAa, pmRAaerr = j[0]['pmra'], j[0]['pmra_error']
pmRAb, pmRAberr = j[1]['pmra'], j[1]['pmra_error']
pmDeca, pmDecaerr = j[0]['pmdec'], j[0]['pmdec_error']
pmDecb, pmDecberr = j[1]['pmdec'], j[1]['pmdec_error']
...
if rank == 0:
    ...
    print 'pmDec, err in mas/yr:',pmDec,pmDecerr
    print 'pmRA, err in mas/yr:',pmRA,pmRAerr
